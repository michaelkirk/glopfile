...
struct FileUploadView: View {
    ...
    var shareView: some View {
        VStack {
            if case .success = fileUpload.state {
            } else if let downloadUrl = fileUpload.pending?.provisionedFile?.formattedDownloadUrlAndKey() {
                ...
                if let image = buildQRImage(urlString: downloadUrl) {
                    ...
                    Image(uiImage: image)
                        ...
                        .frame(minWidth: nil, idealWidth: nil, maxWidth: 200, minHeight: nil, idealHeight: nil, maxHeight: 200, alignment: .center)
                }
            }
        }
    }
